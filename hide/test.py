...
class HideTest(TestCase):
    ...
    def test_multiple_key_provided(self):
        try:
            Hide.objects.create(created_by=self.actor, comment=self.comment, publication=self.publication)
            raise Exception("Hey, it has passed!")
        except Exception as e:
            self.assertEqual("ValidationError", e.__class__.__name__)
    ...
    def test_unique_publication_hide_for_a_user(self):
        try:
            ...
            self.assertEqual(e.__str__(), 'UNIQUE constraint failed: hide_hide.publication_id, hide_hide.created_by_id')